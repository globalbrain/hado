 ...
 * - `fx.all`: For fetching multiple requests concurrently and returning an array
 *   of all results.
 * - `fx.iter`: For fetching multiple requests concurrently and returning an async
 *   iterator that yields results as they complete.
 *
 * All functions automatically handle rate limiting, retries, and timeouts. The
 * pool is shared across all calls using the same `key`.
 *
 * @example Fetching a single request
 *
 * ```ts
 * const request = new Request(
 *   '[https://dummy.restapiexample.com/api/v1/employee/1](https://dummy.restapiexample.com/api/v1/employee/1)'
 * )
 *
 ...
 *
 * ```ts
 * import { object, string, number } from 'jsr:@standard-schema/schema'
 *
 ...
 *   responses).
 * @param input The request object to fetch.
